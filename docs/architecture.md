...
`ausecon-mcp-server` is a thin FastMCP surface over curated ABS, RBA, and APRA retrieval layers.
...
- `src/ausecon_mcp/catalogue/`: Curated ABS, RBA, and APRA metadata plus semantic resolution and search.
- `src/ausecon_mcp/providers/`: Async HTTP clients, retry logic, cache integration, and metadata stamping.
- `src/ausecon_mcp/parsers/`: Pure payload normalisers from upstream CSV/XML/XLSX into the shared response shape.
...
resolution. Raw ABS/RBA/APRA tools deliberately keep source-native date conventions.
...
- Retrieval responses for ABS, RBA, and APRA share one top-level shape, documented in
