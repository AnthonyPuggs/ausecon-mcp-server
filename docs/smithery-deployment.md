...
git ls-remote origin refs/heads/main refs/tags/v1.12.1
python3 -m pip index versions ausecon-mcp-server
```

Expected state for the v1.12.1 Smithery release:

- `main` and `v1.12.1` point at the same published release commit.
- PyPI lists `ausecon-mcp-server (1.12.1)`.
- `server.json` and `CHANGELOG.md` both refer to `1.12.1`.
...
Do not add `configSchema` or `exampleConfig` for v1.12.1. The server does not
...
- Check response metadata from a live retrieval. `metadata.server_version`
  should report `1.12.1`; if it reports a fallback value, configure Smithery to
  pass `AUSECON_VERSION=1.12.1` as a Docker build argument if the platform
  supports build args.
