...
git ls-remote origin refs/heads/main refs/tags/v1.12.0
python3 -m pip index versions ausecon-mcp-server
```

Expected state for the v1.12.0 Smithery release:

- `main` and `v1.12.0` point at the same published release commit.
- PyPI lists `ausecon-mcp-server (1.12.0)`.
- `server.json` and `CHANGELOG.md` both refer to `1.12.0`.
...
Do not add `configSchema` or `exampleConfig` for v1.12.0. The server does not
...
- Confirm Smithery detects the expected fourteen tools: `search_datasets`, `list_catalogue`,
  `list_economic_concepts`, `get_abs_dataset_structure`, `get_abs_data`, `list_rba_tables`,
  `get_rba_table`, `get_apra_data`, `get_economic_series`, `get_derived_series`,
...
- Check response metadata from a live retrieval. `metadata.server_version`
  should report `1.12.0`; if it reports a fallback value, configure Smithery to
  pass `AUSECON_VERSION=1.12.0` as a Docker build argument if the platform
  supports build args.
